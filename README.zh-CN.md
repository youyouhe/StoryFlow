...
- **AI 辅助创作**
  ...
  - 生成创意灵感
  - 重写现有段落
  - 分镜：为动作块（场景插画）和角色块（人物设定）生成文生图提示词——六大结构化视觉要素，按块保存并可复制到出图工具
  - 支持 Gemini 和 DeepSeek 两种 AI 服务，可配置模型及 Gemini 推理深度
...
| `Alt+C` / `Alt+I` / `Alt+R` | AI 续写 / 灵感 / 润色 |
| `Alt+S` | 生成分镜文生图提示词（动作块 / 角色块） |
