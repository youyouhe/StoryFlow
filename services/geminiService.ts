...
Generate the six-line image prompt for the ${targetNoun}. Remember: exactly six labeled lines, English, no technical terms, no markdown.${charDesigns.size && !isEnvironment ? ' Characters in frame MUST match the established designs above.' : ''}`;
  ...
  // (live case: "cozy friendly intimacy" → content_policy_violation on an
  // adult sofa beat). Replace known trigger words with neutral equivalents —
  // same meaning, no checker trip.
  ...
  let safePrompt = lines.join('\n');
  for (const [re, rep] of FILTER_TRIGGERS) safePrompt = safePrompt.replace(re, rep);
