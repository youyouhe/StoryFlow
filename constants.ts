...
export const DEFAULT_APP_SETTINGS: AppSettings = {
  ...
  // Word-level ASR for P2b alignment (OpenAI-compatible transcriptions).
  // Groq default; 2026-09 preflight from the studio LAN returned 403 at the
  // edge — verify from the browser or point at any compatible/self-hosted
  // endpoint. JSON import works with zero services either way.
  asrApiKey: '',
...
export const TRANSLATIONS = {
  en: {
    ...
    projectSection: "Project Directory",
    projectNone: "No project open — content lives in this browser",
    projectCurrent: "Editing from",
    projectOpen: "Open project folder",
    projectReopen: "Switch project folder",
    projectClose: "Close project",
    projectHint: "story.sfstory · style.sfstyle · runs/*.sfrun — plain text, git-friendly",
    projectUnavailable: "This browser has no File System Access API (use Chrome/Edge or the desktop app).",
    timingTab: "Timing",
    timingHint: "Word-level timeline — width ∝ time. Click a word to 校时 it; click its chip again (or the editor) to jump. Select text in the editor to mark a Selection/Moment.",
    timingSourceEstimate: "est.",
    timingSourceAligned: "aligned",
    timingSourceManual: "manual",
    timingConfidence: "conf.",
    timingLowConf: "low confidence",
    timingMarks: "Marks",
    timingNoMarks: "none yet — select text in the editor",
    timingDelete: "delete",
    timingAlign: "Align audio",
    timingAlignBusy: "Aligning…",
    timingImport: "Import ASR JSON",
    timingNoAsrKey: "No ASR key — import JSON or 校时 manually (settings has the key).",
    timingEditWord: "word",
    timingStartLabel: "start",
    timingEndLabel: "end",
    timingSet: "Save",
    timingClear: "Clear",
    timingCancel: "Cancel",
    markBarSelection: "Selection",
    markBarMoment: "Moment",
    markBarName: "name",
    markBarHint: "marks bind effects to words, not seconds",
    asrKeyLabel: "ASR Key (word-level alignment)",
    asrUrlLabel: "ASR Endpoint",
    asrHint: "OpenAI-compatible /audio/transcriptions (Groq whisper-large-v3-turbo by default). Leave empty to use JSON import + manual timing only.",
    credLibraryLabel: "Credential Library",
    credLibraryHint: "Keys live in THIS session's memory only — saves and exports never carry them. Move them across sessions with an encrypted credential file.",
    credExport: "Export encrypted file",
    credImport: "Import file",
    credClear: "Clear session keys",
    profileLabel: "Service bindings",
    profileSourceFile: "storyflow.runtime.json",
    profileSourceDefault: "built-in defaults",
    profileHint: "capability → endpoint. Two endpoints, no binding = submission refused (never a silent fallback). Edit storyflow.runtime.json to switch services.",
    planTitle: "Frozen Plan — review before paying",
    planRunLabel: "Reuse from run",
    planRunNone: "no reuse",
    planRunHint: "two run files with different satisfactions = A/B",
    planSatisfied: "satisfied by candidate",
    planNoRequest: "no request",
    planUnknownCost: "cost unknown",
    planTotals: "known total",
    planEmpty: "no external requests in this plan",
    planConfirm: "Confirm & submit",
    planCancel: "Cancel",
  ...
  zh: {
    ...
    scriptMeta: "剧本信息",
    projectSection: "项目目录",
    projectNone: "未打开项目——内容暂存在浏览器中",
    projectCurrent: "当前项目",
    projectOpen: "打开项目文件夹",
    projectReopen: "更换项目文件夹",
    projectClose: "关闭项目",
    projectHint: "story.sfstory · style.sfstyle · runs/*.sfrun——纯文本，可 git 管理",
    projectUnavailable: "当前浏览器不支持文件系统访问 API（请用 Chrome/Edge 或桌面端）。",
    timingTab: "词级时间",
    timingHint: "词级时间轴——宽度∝时长。点词校时；再点一次芯片或在编辑器选中文本可跳转/建标记（区间/时刻）。",
    timingSourceEstimate: "估时",
    timingSourceAligned: "对齐",
    timingSourceManual: "校时",
    timingConfidence: "置信",
    timingLowConf: "低置信",
    timingMarks: "标记",
    timingNoMarks: "暂无——在编辑器选中文本创建",
    timingDelete: "删除",
    timingAlign: "对齐音频",
    timingAlignBusy: "对齐中…",
    timingImport: "导入 ASR JSON",
    timingNoAsrKey: "未配 ASR 密钥——可导入 JSON 或手动校时（密钥在设置里）。",
    timingEditWord: "词",
    timingStartLabel: "起点",
    timingEndLabel: "终点",
    timingSet: "保存",
    ...
    markBarMoment: "时刻",
    markBarName: "名称",
    markBarHint: "标记把效果绑到词上，而不是秒",
    asrKeyLabel: "ASR 密钥（词级对齐）",
    asrUrlLabel: "ASR 接口地址",
    asrHint: "OpenAI 兼容 /audio/transcriptions（默认 Groq whisper-large-v3-turbo）。留空则只用 JSON 导入 + 手动校时。",
    credLibraryLabel: "凭证库",
    credLibraryHint: "密钥只存本次会话内存——保存与导出永不携带。跨会话用口令加密的凭证文件迁移。",
    credExport: "导出加密凭证",
    credImport: "导入凭证",
    credClear: "清空会话密钥",
    profileLabel: "服务绑定",
    profileSourceFile: "storyflow.runtime.json",
    profileSourceDefault: "内置默认",
    profileHint: "能力 → 端点。两个端点不绑定 = 提交被拒（绝不静默改道）。改 storyflow.runtime.json 即换服务商。",
    planTitle: "冻结计划——付款前审阅",
    planRunLabel: "复用来源 Run",
    planRunNone: "不复用",
    planRunHint: "两个 satisfy 不同的 run 文件即 A/B 对照",
    planSatisfied: "由候选满足",
    planNoRequest: "不发请求",
    planUnknownCost: "费用未知",
    planTotals: "已知合计",
    planEmpty: "本计划没有外部请求",
    planConfirm: "确认提交",
    planCancel: "取消",
